# coding:utf-8
from extract_data import extract_data, extract_data_visit, extract_data_form, extract_data_new
...
def extract_handler_new(path, method_mode, form_id=None, function_id=None):
    page_number = 1
    extract_form = False
    if form_id:
        lv_path = path + '-' + str(form_id)
    ...
    while go_on:
        ...
        data = extract_data_new(path=path, page_number=page_number,
                                after_modify_date=after_modify_date,
                                method_mode=method_mode, before_modify_date=before_modify_date,
                                form_id=form_id, function_id=function_id)
